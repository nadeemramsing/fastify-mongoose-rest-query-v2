...
import { getArrayFromBodyWithId, runStaticMethods, useSession, } from '../utils/mongoose.utils';
import { DOCUMENT_NOT_FOUND, IMPLICIT_DELETE_ALL_NOT_ALLOWED, INVALID_BODY, NO_DOCUMENT_FOUND, PATH_NOT_FOUND_IN_SCHEMA, ROLE_DOES_NOT_HAVE_ACCESS_HANDLER_LEVEL, } from '../mrq.errors';
export const getMainHandler = (modelName, handlerAccesses = []) => {
    ...
    const distinct = async (req, rep) => {
        ...
        if (!doesPathExists)
            throw httpErrors.notFound(PATH_NOT_FOUND_IN_SCHEMA);
        const query = getQuery(req, modelName, { ignoreSelect: true });
    ...
    const create = async (req, rep) => {
        ...
        if (!isBodyAnArray)
            body = [body];
        const docs = await useSession(Model, req, (session) => Model.create(body, { req, session, ordered: !!session }));
    ...
    const updateMany = async (req, rep) => {
        ...
        const docs = await Model.find({ _id }).then((docs) => docs.reduce((acc, v) => ({ ...acc, [v._id]: v }), {}));
        //@ts-ignore
    ...
    const deleteByQuery = async (req, rep) => {
        ...
        if (isDeleteAll)
            throw httpErrors.methodNotAllowed(`${IMPLICIT_DELETE_ALL_NOT_ALLOWED}: delete directly in database`);
        const { deletedCount } = await useSession(Model, req, (session) => Model.deleteMany(query.filter, { session }));
        if (!deletedCount)
            throw httpErrors.notFound(NO_DOCUMENT_FOUND);
        return { deletedCount };
    };
    // ---
    const getById = async (req, rep) => {
        ...
        const doc = await Model.findById(id, query.select, { req })
            ...
            .lean(leanOptions);
        if (!doc)
            throw httpErrors.notFound(DOCUMENT_NOT_FOUND);
    ...
    const deleteById = async (req, rep) => {
        ...
        const { deletedCount } = await useSession(Model, req, (session) => Model.deleteOne({ _id }, { session }));
        if (!deletedCount)
            throw httpErrors.notFound(DOCUMENT_NOT_FOUND);
        return { deletedCount };
    };
