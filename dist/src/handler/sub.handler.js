...
export const getSubHandler = (modelName, { subPathName, handlerAccesses, getSubarray, subIdName, }) => {
    ...
    const create = async (req, rep) => {
        if (!handlerAccesses.includes(HandlerAccessEnum.CREATE_SUB))
            throw httpErrors.unauthorized(ROLE_DOES_NOT_HAVE_ACCESS_HANDLER_LEVEL);
        const { Model, doc, subarray } = await getSubarray(req, modelName, subPathName);
