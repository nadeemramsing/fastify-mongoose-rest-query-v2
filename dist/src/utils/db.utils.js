...
const pool = {};
export async function getDB(app, uri, schemas) {
    let conn = pool[uri];
    if (!conn) {
...
async function mapModels(app, conn, schemas) {
    ...
    const diffs = await promiseAll(p).then(pickBy((v, k) => v.toDrop.length || v.toCreate.length));
...
export async function closeConnections() {
    const p = [];
    for (const uri in pool)
        p.push(pool[uri].close());
    await Promise.allSettled(p);
}
export function model(req, modelName) {
    const Model = req.mongoose_conn.models[modelName];
    if (!Model)
        throw httpErrors.badRequest(SCHEMA_NOT_REGISTERED);
    return Model;
}
