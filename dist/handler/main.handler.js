...
var getMainHandler = (modelName, handlerAccesses = []) => {
  const getByQuery = async (req, rep) => {
    if (!handlerAccesses.includes(HandlerAccessEnum.GET_BY_QUERY))
      throw import_sensible.httpErrors.unauthorized(ROLE_DOES_NOT_HAVE_ACCESS_HANDLER_LEVEL);
  ...
  const distinct = async (req, rep) => {
    ...
    if (!doesPathExists) throw import_sensible.httpErrors.notFound(PATH_NOT_FOUND_IN_SCHEMA);
    const query = getQuery(req, modelName, { ignoreSelect: true });
  ...
  const create = async (req, rep) => {
    ...
    if (isBodyAnArray && !body.length)
      throw import_sensible.httpErrors.notFound(`${INVALID_BODY}: no object found in array`);
  ...
  const deleteByQuery = async (req, rep) => {
    ...
    if (!deletedCount) throw import_sensible.httpErrors.notFound(NO_DOCUMENT_FOUND);
    return { deletedCount };
  };
  ...
  const deleteById = async (req, rep) => {
    ...
    if (!deletedCount) throw import_sensible.httpErrors.notFound(DOCUMENT_NOT_FOUND);
    return { deletedCount };
  };
  return {
    getByQuery,
