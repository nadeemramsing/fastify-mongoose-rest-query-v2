...
describe.sequential('/ PUT (updateMany > Error handling)', () => {
    it('should return "EMPTY_BODY" error when empty array used', async () => {
        ...
        expect(response.json().message).toMatch(/^EMPTY_BODY: body should contain at least one object/);
    });
    it('should return "INVALID_BODY" error when array with 1 empty object used', async () => {
        ...
        expect(response.statusCode).toBe(422);
        expect(response.json().message).toMatch(/^INVALID_BODY: body should contain at least one object with _id/);
    });
    it('should return "INVALID_BODY" error when object used', async () => {
        ...
        expect(response.statusCode).toBe(422);
        expect(response.json().message).toMatch(/^INVALID_BODY: body should be an array/);
    });
