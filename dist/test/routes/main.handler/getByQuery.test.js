...
import { drop, filter, find, map, orderBy, pick, pipe, take, forEach, } from 'lodash/fp';
describe.sequential('/ GET (getByQuery)', () => {
    ...
    it('should return all resources with no filter', async () => {
        ...
        const resourcesExpected = getDocsInJSON(resources).map((resource) => {
    ...
    it('should return resources with 1 filter name=Nadeem', async () => {
        ...
        const resourcesExpected = pipe(filter({ name: 'Nadeem' }), forEach((resource) => {
            delete resource.auth?.password;
    ...
    it('should return resources with 2 filters name=~mira&age>=10', async () => {
        ...
        const resourcesExpected = pipe(filter((doc) => RegExp('mira', 'i').test(doc.name) && doc.age >= 10), forEach((resource) => {
            delete resource.auth?.password;
        }))(getDocsInJSON(resources));
        expect(response.statusCode).toBe(200);
        expect(response.json()).toMatchObject(resourcesExpected);
    ...
    it('should return resources with 1 filter addresses.street=street3', async () => {
        ...
        const resourcesExpected = pipe(filter((doc) => doc.addresses.find((addr) => addr.street === 'street3')), forEach((resource) => {
            delete resource.auth?.password;
        }))(getDocsInJSON(resources));
        expect(response.statusCode).toBe(200);
        expect(response.json()).toMatchObject(resourcesExpected);
    ...
    it('should return resources with 1 filter noField=null (i.e. all)', async () => {
        ...
        const resourcesExpected = pipe(filter((doc) => !doc.noField), forEach((resource) => {
            delete resource.auth?.password;
        }))(getDocsInJSON(resources));
        expect(response.statusCode).toBe(200);
        expect(response.json()).toMatchObject(resourcesExpected);
    ...
    it('should return all resources with selected fields name,age', async () => {
        ...
        const resourcesExpected = map(pick(['_id', 'name', 'age']), getDocsInJSON(resources));
        expect(response.statusCode).toBe(200);
        expect(response.json()).toMatchObject(resourcesExpected);
    });
    it('should return all resources with selected fields name, age', async () => {
    ...
    it('should return all resources with selected fields name, age, addresses and selected static method firstCity', async () => {
        ...
        const resourcesExpected = map(pick(['_id', 'name', 'age', 'addresses']), getDocsInJSON(resources));
        resourcesExpected.forEach((doc) => {
            const firstCity = doc.addresses.find((address) => address.flags?.is_a_city)?.city;
            if (firstCity)
                doc.firstCity = firstCity;
        });
        expect(response.statusCode).toBe(200);
        expect(response.json()).toMatchObject(resourcesExpected);
    });
    ...
    it('should return all resources sorted by age', async () => {
        ...
        const resourcesExpected = pipe(orderBy('age', 'asc'), forEach((resource) => {
            delete resource.auth?.password;
        }))(getDocsInJSON(resources));
        expect(response.statusCode).toBe(200);
        expect(response.json()).toMatchObject(resourcesExpected);
    ...
    it('should return 2 first resources with limit 2', async () => {
        ...
        const resourcesExpected = pipe(take(2), forEach((resource) => {
            delete resource.auth?.password;
        }))(getDocsInJSON(resources));
        expect(response.statusCode).toBe(200);
        expect(response.json()).toMatchObject(resourcesExpected);
