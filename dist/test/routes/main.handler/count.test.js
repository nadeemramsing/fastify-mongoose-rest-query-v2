...
describe.sequential('/ GET (count)', () => {
    ...
    it('should return count of resources with 1 filter name=Nadeem', async () => {
        ...
        const resourcesExpected = filter({ name: 'Nadeem' }, getDocsInJSON(resources));
        expect(response.statusCode).toBe(200);
        expect(response.json().n).toBe(resourcesExpected.length);
    });
    it('should return count of resources with 2 filters name=~mira&age>=10', async () => {
        ...
        const resourcesExpected = filter((doc) => RegExp('mira', 'i').test(doc.name) && doc.age >= 10, getDocsInJSON(resources));
        expect(response.statusCode).toBe(200);
        expect(response.json().n).toBe(resourcesExpected.length);
    });
    it('should return count of resources with 1 filter addresses.street=street3', async () => {
        ...
        const resourcesExpected = filter((doc) => doc.addresses.find((addr) => addr.street === 'street3'), getDocsInJSON(resources));
        expect(response.statusCode).toBe(200);
        expect(response.json().n).toBe(resourcesExpected.length);
    });
    it('should return count of resources with 1 filter noField=null (i.e. all)', async () => {
        ...
        const resourcesExpected = filter((doc) => !doc.noField, getDocsInJSON(resources));
        expect(response.statusCode).toBe(200);
        expect(response.json().n).toBe(resourcesExpected.length);
    });
    it('should return count of resources with 1 filter noField=any_value (i.e. none)', async () => {
        ...
        const resourcesExpected = filter((doc) => doc.noField === 'any_value', getDocsInJSON(resources));
        expect(response.statusCode).toBe(200);
        expect(response.json().n).toBe(resourcesExpected.length);
    });
});
