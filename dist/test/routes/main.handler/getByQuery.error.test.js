...
describe.sequential('/ GET (getByQuery > Error handling)', () => {
    it('should return 405 on trying to select all without explicitly selecting all', async () => {
        ...
        expect(response.json().message).toMatch(/^IMPLICIT_SELECT_ALL_NOT_ALLOWED: use select=all query param/);
    });
    it('should return 404 on calling non-existing endpoint', async () => {
        ...
        expect(response.json().message).toMatch(/^Cast to ObjectId failed for value \"non-existing\" \(type string\) at path \"_id\" for model \"Resource\"/);
    });
    ...
    it('should return error when non-existing populate field is used', async () => {
        ...
        expect(response.statusCode).toBe(500);
        expect(response.json().message).toMatch(/^Cannot populate path `non-existing` because it is not in your schema\. Set the `strictPopulate` option to false to override\.$/);
    });
});
